...
export function SolarProjects() {
  return (
    <motion.section
      className="mx-auto mt-[clamp(34px,5vw,72px)] w-[min(1320px,calc(100%-2*var(--gutter)))] overflow-hidden rounded-xl border border-slate-200 bg-white/95 px-[clamp(16px,4vw,46px)] py-[clamp(34px,4vw,56px)] shadow-[0_24px_58px_rgba(8,52,107,.1)]"
    ...
    >
      <header className="mb-6 max-w-180">
        ...
        <h2 className="my-3 text-[clamp(32px,4.2vw,56px)] font-bold leading-none tracking-[-.055em] text-[#08346b]">Our Solar Projects</h2>
      </header>
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3 lg:gap-3.5">
        {solarProjects.map((project) => (
          <figure className="m-0 overflow-hidden rounded-xl border border-slate-200 bg-white shadow-[0_10px_28px_rgba(8,52,107,.08)]" key={project.image}>
            <img className="block h-48 w-full object-cover sm:h-44 lg:h-[clamp(180px,18vw,240px)]" src={project.image} alt={project.title} loading="lazy" />
